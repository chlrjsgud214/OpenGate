...
<script type="text/javascript">
	function noticeReplyDelete(bno, rno) {
		...
		$.post('noticeReplyDelete', sendData, function(data) {
			alert("댓글이 삭제 되었습니다");
			$('#noticeReplyList').html(data);
		});
	}
	function noticeReplyUpdate(bno, rno) {
		var txt = $('#td_'+rno).text();
		$('#td_'+rno).html("<textarea rows='3' cols='90' id='nrt'>"+txt+"</textarea>");
	...
	function up(bno, rno) {
		...
		$.post('noticeReplyUpdate', sendData, function(data){
			alert('수정 되었습니다');
			$('#noticeReplyDisp').html(data);
		});
	}
	function lst(bno) {
		$('#noticeReplyDisp').load('/notice/noticeReplyList/bno'+bno);
	}
</script>
</head>
<body>
		...
		<c:forEach var="nrbd" items="${nReplyList }">
			...
			<c:if test="${nrbd.del != 'y'}">
				<tr>
					...
					<c:if test="${nrbd.replyer == notice.writer || notice.writer == 'master' }">
						<td id="btn_${nrbd.rno }">
